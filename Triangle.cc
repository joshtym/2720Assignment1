...
/// Item counter across all objects
/**
 * Keeps track of all the triangle objects made
**/
...
void Triangle::draw(double originx, double originy)
{
	...
	while (startingDegree != degreeRotation)
		startingDegree += degreeRotation;
	startingDegree -= 360/numOfTriangles;
	
	p1.rotate(origin, p1, sideLength, startingDegree);
	
	p2.rotate(origin, p2, sideLength, degreeRotation);
		
	al_draw_triangle(origin.getX(), origin.getY(), p1.getX(),
			         p1.getY(), p2.getX(), p2.getY(),
					 al_map_rgb(49,94,199), 1);
}
